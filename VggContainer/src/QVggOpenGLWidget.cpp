...
public:
  QVggOpenGLWidgetImpl(QVggOpenGLWidget *api)
      : m_api{api}, m_container{new VGG::QtContainer} {
    m_animator.setInterval(16);
  }

  // === GL ============================================================
  void initializeGL() { m_funcs.initializeOpenGLFunctions(); }
  ...
  void paintGL() { m_container->paint(true); }
  ...
  bool load(const std::string &filePath,
    ...
    return m_container->load(filePath, designDocSchemaFilePath,
  ...
  void setEventListener(QVggOpenGLWidget::EventListener listener) {
    if (listener) {
      ...
      m_container->setEventListener([listener, sdk](std::string type,
                                                    std::string targetId,
...
QVggOpenGLWidget::QVggOpenGLWidget(QWidget *parent)
    : QOpenGLWidget(parent), m_impl(new QVggOpenGLWidgetImpl(this)) {
  QObject::connect(&m_impl->m_animator, &QTimer::timeout, this, [this]() {
...
// === api ===============================================================
bool QVggOpenGLWidget::load(const std::string &filePath,
                            const char *designDocSchemaFilePath,
                            const char *layoutDocSchemaFilePath) {
  return m_impl->load(filePath, designDocSchemaFilePath,
                      layoutDocSchemaFilePath);
}
...
// === private ===============================================================
void QVggOpenGLWidget::init(int w, int h) { m_impl->init(w, h); }
